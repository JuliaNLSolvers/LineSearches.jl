...
HagerZhang{T}(args...; kwargs...) where {T} =
    HagerZhang{T,Base.RefValue{Bool}}(args...; kwargs...)

function (ls::HagerZhang)(
    df::AbstractObjective,
    x::AbstractArray{T},
    s::AbstractArray{T},
    α::Real,
...
function (ls::HagerZhang)(ϕ, ϕdϕ, c::T, phi_0::Real, dphi_0::Real) where {T} # Should c and phi_0 be same type?
    @unpack delta,
    sigma,
    alphamax,
    rho,
    ...
    while !isbracketed && iter < linesearchmax
        ...
        elseif values[end] > phi_lim
            ...
            if c > alphamax
                ...
                if display & BRACKET > 0
                    println(
                        "bracket: exceeding alphamax, using c = alphamax = ",
                        alphamax,
                        ", cold = ",
                        cold,
                    )
            ...
            if !(isfinite(phi_c) && isfinite(dphi_c))
                if display & BRACKET > 0
                    println(
                        "Warning: failed to expand interval to bracket with finite values. If this happens frequently, check your function and gradient.",
                    )
                    println(
                        "c = ",
                        c,
    ...
    while iter < linesearchmax
        ...
        iswolfe, iA, iB =
            secant2!(ϕdϕ, alphas, values, slopes, ia, ib, phi_lim, delta, sigma, display)
        if iswolfe
            mayterminate[] = false # reset in case another initial guess is used next
        ...
        if B - A < gamma * (b - a)
            ...
            if ls.check_flatness &&
               nextfloat(values[ia]) >= values[ib] &&
               nextfloat(values[iA]) >= values[iB]
